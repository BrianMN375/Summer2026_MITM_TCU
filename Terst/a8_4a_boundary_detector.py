#!/usr/bin/env python3

import csv
from collections import Counter

CSV_FILE = (
    "/Users/brianmay/Dropbox/"
    "Electronics Dev Library and Docs/"
    "A2 - PlatformIO/"
    "PlatformIO Repository/"
    "Github Repositories/"
    "Summer2026_MITM_TCU/"
    "Summer2026_MITM_TCU/"
    "logs/"
    "a8_full_0007.csv"
)

COUNTER_TRANSFORM = [
    0x00,0x7A,0x96,0x79,
    0xD5,0xBF,0x8F,0x9A,
    0xB8,0xCA,0x95,0x58,
    0x46,0x55,0xE7,0x68
]

KNOWN_VECTORS = {
    (2,5):0x9B,(2,6):0x19,(2,7):0x32,

    (3,0):0x54,(3,1):0xA8,
    (3,2):0x7F,(3,3):0xFE,
    (3,6):0x3D,(3,7):0x7A,

    (4,0):0xAD,(4,1):0x75,
    (4,2):0xEA,(4,3):0xFB,

    (6,1):0x1C,(6,2):0x38,
    (6,3):0x70,(6,4):0xE0,
    (6,5):0xEF,(6,6):0xF1,
    (6,7):0xCD,

    (7,0):0xE9,(7,1):0xFD,
    (7,2):0xD5,(7,3):0x85,
    (7,4):0x25,
}

# ------------------------------------------------------------

def build_u64(frame):

    value = 0

    for i, b in enumerate(frame):
        value |= (b << (8 * i))

    return value

# ------------------------------------------------------------

def decode_signals(frame):

    raw = build_u64(frame)

    rpm  = ((raw >> 48) & 0xFFFF) * 0.25
    stat = ((raw >> 21) & 0x1FF)
    dyn  = ((raw >> 30) & 0x3FF) - 509

    return rpm, stat, dyn

# ------------------------------------------------------------

def actual_residual(frame):

    checksum = frame[0]
    counter  = frame[1] & 0x0F

    family = checksum ^ COUNTER_TRANSFORM[counter]

    linear = 0

    for byte_idx in range(2,8):

        value = frame[byte_idx]

        for bit_idx in range(8):

            if value & (1 << bit_idx):

                vec = KNOWN_VECTORS.get(
                    (byte_idx, bit_idx)
                )

                if vec is not None:
                    linear ^= vec

    return family ^ linear

# ------------------------------------------------------------

def predicted_residual(frame):

    byte4 = frame[4]
    byte7 = frame[7]

    rpm, stat, dyn = decode_signals(frame)

    if byte4 == 0xFF:
        return 0x0D

    if 0xB0 <= byte4 <= 0xBF:
        return 0x5D

    if 0xA0 <= byte4 <= 0xAF:
        return 0x84

    if 0x99 <= byte4 <= 0x9F and byte7 >= 0x40:
        return 0xC0

    if 0x90 <= byte4 <= 0x9F:
        return 0x1E

    neg = dyn < 3
    high = rpm >= 2050
    mid = stat >= 128

    if not high:

        if not mid:
            return 0x25 if neg else 0x5E

        return 0xF6 if neg else 0x8D

    else:

        if neg:
            return 0xBC

        return 0xC7

# ------------------------------------------------------------

rows_4a = []

print()
print("Loading dataset...")

with open(CSV_FILE, newline="") as csvfile:

    reader = csv.reader(csvfile)

    for row in reader:

        if not row:
            continue

        if row[0].lower() == "time":
            continue

        try:
            frame = [
                int(x.strip(),16)
                for x in row[1:9]
            ]
        except:
            continue

        actual = actual_residual(frame)
        predicted = predicted_residual(frame)

        if (actual ^ predicted) != 0x4A:
            continue

        rpm, stat, dyn = decode_signals(frame)

        rows_4a.append(
            (frame, rpm, stat, dyn, actual, predicted)
        )

# ------------------------------------------------------------

print()
print("=" * 80)
print("A8 0x4A BOUNDARY DETECTOR")
print("=" * 80)

print()
print(f"Frames : {len(rows_4a):,}")

rpms  = [r[1] for r in rows_4a]
stats = [r[2] for r in rows_4a]
dyns  = [r[3] for r in rows_4a]

print()
print(
    f"RPM  : {min(rpms):.0f}..{max(rpms):.0f} "
    f"avg={sum(rpms)/len(rpms):.2f}"
)

print(
    f"STAT : {min(stats)}..{max(stats)} "
    f"avg={sum(stats)/len(stats):.1f}"
)

print(
    f"DYN  : {min(dyns)}..{max(dyns)} "
    f"avg={sum(dyns)/len(dyns):.1f}"
)

# ------------------------------------------------------------
# RPM Histogram
# ------------------------------------------------------------

rpm_bins = Counter()

for _, rpm, _, _, _, _ in rows_4a:

    rpm_bins[int(rpm)] += 1

print()
print("RPM Distribution")
print("----------------")

for rpm in sorted(rpm_bins):

    print(
        f"{rpm:4d} : {rpm_bins[rpm]}"
    )

# ------------------------------------------------------------
# Byte4 ownership
# ------------------------------------------------------------

b4 = Counter(r[0][4] for r in rows_4a)

print()
print("Byte4 Distribution")
print("------------------")

for value,count in b4.most_common(20):

    print(
        f"0x{value:02X} "
        f"{count:6d}"
    )

# ------------------------------------------------------------
# Actual / Predicted
# ------------------------------------------------------------

pairs = Counter()

for _, _, _, _, actual, predicted in rows_4a:

    pairs[(actual,predicted)] += 1

print()
print("Actual -> Predicted")
print("-------------------")

for (actual,predicted),count in pairs.most_common():

    print(
        f"0x{actual:02X} -> 0x{predicted:02X} "
        f"{count}"
    )

# ------------------------------------------------------------
# Sample frames
# ------------------------------------------------------------

print()
print("Example Frames")
print("--------------")

for frame,rpm,stat,dyn,actual,predicted in rows_4a[:20]:

    print(
        f"RPM={rpm:7.2f} "
        f"STAT={stat:3d} "
        f"DYN={dyn:4d} "
        f"ACT=0x{actual:02X} "
        f"PRED=0x{predicted:02X} "
        + " ".join(f"{b:02X}" for b in frame)
    )

print()
print("Done.")